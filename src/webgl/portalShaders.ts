...
void mainImage(out vec4 fc, in vec2 fCoord) {
  ...
  float portalProgress = S(0., 1., u_transitionProgress);
  float portalEase = 1. - pow(1. - portalProgress, 3.);
  vec2 portalDelta = UV - u_transitionOrigin;
  portalDelta.x *= aspect;
  float portalDist = length(portalDelta);
  float portalMax = sqrt(aspect * aspect + 1.) * .78;
  float portalRadius = portalEase * portalMax;
  float portalLag = portalRadius - portalDist;
  float portalTrail = step(0., portalLag)
    * (1. - S(0., .34, portalLag))
    * exp(-max(portalLag, 0.) * 2.8);
  float portalWave = sin(portalLag * 108.) * portalTrail;
  portalWave += sin(portalLag * 57. + .8) * portalTrail * .24;
  portalWave *= 1. - portalProgress * .5;
  float portalCrest = exp(-pow((portalDist - portalRadius) * 58., 2.))
    * (1. - portalProgress * .72);
  vec2 portalDir = portalDelta / max(portalDist, .001);
  portalDir.x /= aspect;
  vec2 portalN = portalDir * (
    portalWave * .032
    + portalCrest * .004
  );
  float portalWake = 1. - S(
    portalRadius - .24,
    portalRadius + .02,
    portalDist
  );

  n += clamp(rippleN + portalN, vec2(-.048), vec2(.048));
  ...
  col += portalCrest * vec3(.08, .1, .12) * .34;
  col += abs(portalWave) * vec3(.025, .035, .042) * .16;
  col = mix(col, col * vec3(.95, .98, 1.025), portalWake * .07);
